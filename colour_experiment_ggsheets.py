...
def get_google_sheets_client():
    ...
    try:
        ...
        if "gcp_service_account" in st.secrets:
            ...
            st.error("⚠️ Google Sheets credentials not configured. Please add them to Streamlit secrets.")
            return None
...
def get_or_create_worksheet():
    """Get or create the worksheet for storing results"""
    try:
        client = get_google_sheets_client()
        if not client:
            return None
        
        # Get spreadsheet ID from secrets or use default name
        if "spreadsheet_id" in st.secrets:
            spreadsheet = client.open_by_key(st.secrets["spreadsheet_id"])
        elif "spreadsheet_name" in st.secrets:
            spreadsheet = client.open(st.secrets["spreadsheet_name"])
        else:
            st.error("⚠️ Please add 'spreadsheet_id' or 'spreadsheet_name' to Streamlit secrets.")
            return None
        ...
        try:
            ...
            worksheet = spreadsheet.add_worksheet(title="Results", rows=1000, cols=12)
            # Add headers
            headers = [
                'participant_name', 'gender', 'age', 'sleep_hours',
                'overall_trial', 'spectrum', 'percentage_complete',
                'hex_code', 'rgb', 'reaction_time_ms', 'false_alarm', 'timestamp'
            ]
...
def save_result_to_sheets(result):
    """Save a single result to Google Sheets"""
    try:
        worksheet = get_or_create_worksheet()
        if not worksheet:
            return False
        
        # Convert result dict to row
        row = [
            result.get('participant_name', ''),
            result.get('gender', ''),
            result.get('age', ''),
            result.get('sleep_hours', ''),
            result.get('overall_trial', ''),
            result.get('spectrum', ''),
            result.get('percentage_complete', ''),
            result.get('hex_code', ''),
            result.get('rgb', ''),
            result.get('reaction_time_ms', ''),
            result.get('false_alarm', ''),
...
def load_all_results():
    ...
    try:
        ...
        if 'overall_trial' in df.columns:
            df['overall_trial'] = pd.to_numeric(df['overall_trial'], errors='coerce')
        if 'percentage_complete' in df.columns:
            df['percentage_complete'] = pd.to_numeric(df['percentage_complete'], errors='coerce')
        if 'reaction_time_ms' in df.columns:
            df['reaction_time_ms'] = pd.to_numeric(df['reaction_time_ms'], errors='coerce')
        if 'false_alarm' in df.columns:
            df['false_alarm'] = df['false_alarm'].astype(bool)
...
def calculate_reaction_time(trial_start_time, button_press_time):
    ...
    if trial_start_time and button_press_time:
        return round((button_press_time - trial_start_time) * 1000, 2)  # milliseconds
    return None
...
def run_colour_trial(start_color, end_color, is_practice):
    ...
    if not st.session_state.button_disabled and not st.session_state.trial_complete:
        ...
        if button_pressed:
            ...
            is_false_alarm = reaction_time < 1000 if reaction_time else False
...
def render_results_phase():
    ...
    # Load ALL accumulated data from all users
    all_data_df = load_all_results()
    ...
    with col1:
        st.metric("Your Trials", len(st.session_state.spectra_order))
    with col2:
        st.metric("Duration", f"{duration_minutes:.1f} min")
    
    st.markdown("---")
    
    # Show current user's results
    if st.session_state.all_results:
        ...
        # Statistical summary for current user
        st.subheader("📉 Your Statistical Analysis")
    ...
    # Show aggregate statistics from all users
    st.markdown("---")
    st.subheader("🌍 Aggregate Data from All Participants")
    
    if not all_data_df.empty:
        total_participants = all_data_df['participant_name'].nunique()
        total_trials = len(all_data_df)
        ...
        with col3:
            valid_all = all_data_df[all_data_df['false_alarm'] == False]
            st.metric("Valid Trials", len(valid_all))
        
        # Show aggregate statistics
        if len(valid_all) > 0:
            st.markdown("**Aggregate Statistics** *(all participants, excluding false alarms)*")
            col1, col2 = st.columns(2)
            
            with col1:
                st.write(f"- Mean Detection: {valid_all['percentage_complete'].mean():.1f}%")
                st.write(f"- Median Detection: {valid_all['percentage_complete'].median():.1f}%")
                st.write(f"- Std Dev: {valid_all['percentage_complete'].std():.1f}%")
            
            with col2:
                rt_all = valid_all[valid_all['reaction_time_ms'].notna()]['reaction_time_ms']
                if len(rt_all) > 0:
                    st.write(f"- Mean RT: {rt_all.mean():.0f} ms")
                    st.write(f"- Median RT: {rt_all.median():.0f} ms")
                    st.write(f"- Std Dev: {rt_all.std():.0f} ms")
    else:
        st.info("No accumulated data yet. You are the first participant!")
    
    # Download options - ALL DATA
    st.markdown("---")
    ...
    if not all_data_df.empty:
        ...
        with col1:
            ...
            all_data_df.to_excel(excel_buffer, index=False, engine='openpyxl')
            excel_buffer.seek(0)
            
            st.download_button(
                label="📥 Download ALL Data (Excel)",
                data=excel_buffer,
                file_name=f"colour_perception_ALL_PARTICIPANTS_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True,
                help="Download complete dataset from all participants"
            )
        
        with col2:
            # JSON export - ALL DATA
            json_data = all_data_df.to_json(orient='records', indent=2)
            st.download_button(
                label="📄 Download ALL Data (JSON)",
                data=json_data,
                file_name=f"colour_perception_ALL_PARTICIPANTS_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                mime="application/json",
                use_container_width=True,
                help="Download complete dataset from all participants"
            )
        
        st.info(f"💡 Download includes data from **{all_data_df['participant_name'].nunique()} participants** with **{len(all_data_df)} total trials**")
    else:
        st.warning("No data available for download yet.")
